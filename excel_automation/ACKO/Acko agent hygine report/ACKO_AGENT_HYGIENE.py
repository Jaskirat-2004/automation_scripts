...
email_productivity_dump = email_productivity_dump[['Date','Ticket ','Name','Status','Email Address']]
...
try:
    ...
    try:
        ...
        for col in ["Start Time", "End Time"]:
            matched_dump[col] = pd.to_datetime(matched_dump[col], dayfirst=True, errors="coerce")
    ...
    try:
        ...
        login_logout_sheet[f"E{next_row}"].options(pd.DataFrame,header=False,index=False, expand="table").value = matched_dump
...
try:
    ...
    last_row_IVR_Dump = ws1.Cells(ws1.Rows.Count, 7).End(-4162).Row 
    ...
    formula_range2 = ws2.Range(f"H{last_row2}:I{last_row}")  # Expanding H{last_row2}:I down to the last used row
    formula_range1.FillDown()
    ...
    last_row = ws3.Cells(ws3.Rows.Count, 5).End(-4162).Row
    
    formula_range1 = ws3.Range(f"A{last_row3}:D{last_row}")  # Expanding A2:D down to the last used row
    formula_range2 = ws3.Range(f"N{last_row3}:P{last_row}")  # Expanding N2:P down to the last used row
    formula_range1.FillDown()
    formula_range2.FillDown()
    
    print(f"🎯 Formulas successfully applied to range A{last_row3}:D{last_row} & N{last_row3}:P{last_row}")
    ...
    last_row = ws4.Cells(ws4.Rows.Count, 30).End(-4162).Row 
    
    formula_range1 = ws4.Range(f"A{last_row4}:AB{last_row}")  # Expanding A{last_row4}:AB down to the last used row
    formula_range1.FillDown()
    print(f"🎯 Formulas successfully applied to range A{last_row4}:AB{last_row}")
